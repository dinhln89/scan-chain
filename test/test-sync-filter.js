...
const SYNC_TOPICS = new Set([SYNC_TOPIC_V2, SYNC_TOPIC_V3]);
...
const makeBalanceOfCall = (token, wallet) => ({ fn: "balanceOf(address)", to: token, wallet });
...
test("V2: token emit Sync → bi loai (BUSD case)", () => {
  const set = new Set([BUSD, TOKEN_X]);
  filterSync(set, [{ topics: [SYNC_TOPIC_V2], address: BUSD }], []);
  assert.ok(!set.has(BUSD));
  assert.ok(set.has(TOKEN_X));
});

test("V3: token emit Sync → bi loai", () => {
  const set = new Set([BUSD, TOKEN_X]);
...
test("nhieu token emit Sync → tat ca bi loai", () => {
  ...
  filterSync(set, [
    { topics: [SYNC_TOPIC_V2], address: BUSD },
    { topics: [SYNC_TOPIC_V3], address: PAIR_A },
  ], []);
  assert.ok(!set.has(BUSD));
...
test("log khong co topics → bo qua", () => {
  ...
  filterSync(set, [{ address: BUSD }, { topics: [], address: BUSD }, { topics: null, address: BUSD }], []);
  assert.ok(set.has(BUSD));
});
...
// --- Tầng 2: pair (c.wallet) emit Sync → filter tokens của pair ---
console.log("\n[Tang 2: pair emit Sync → filter tokens trong pair (AW/FIST case)]");

test("V2: pair emit Sync → token cua pair bi loai (AW/FIST)", () => {
  const set = new Set([AW, FIST, TOKEN_X]);
  filterSync(set, [{ topics: [SYNC_TOPIC_V2], address: PAIR_B }], [
    makeBalanceOfCall(AW, PAIR_B),
    makeBalanceOfCall(FIST, PAIR_B),
  ]);
...
test("V3: pair emit Sync → token cua pair bi loai", () => {
  const set = new Set([AW, FIST, TOKEN_X]);
  filterSync(set, [{ topics: [SYNC_TOPIC_V3], address: PAIR_B }], [
    makeBalanceOfCall(AW, PAIR_B),
    makeBalanceOfCall(FIST, PAIR_B),
  ]);
  assert.ok(!set.has(AW));
...
test("pair khong emit Sync → tokens giu nguyen", () => {
  ...
  filterSync(set, [{ topics: ["0xother"], address: PAIR_B }], [
    makeBalanceOfCall(AW, PAIR_B),
    makeBalanceOfCall(FIST, PAIR_B),
  ]);
  assert.ok(set.has(AW));
...
test("token trong pair khac emit Sync → khong anh huong", () => {
  ...
  filterSync(set, [{ topics: [SYNC_TOPIC_V2], address: PAIR_A }], [
    makeBalanceOfCall(AW, PAIR_B),
  ]);
